...
export default function AddHabitForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
      <h2 className="font-medium text-gray-900">Nový zvyk</h2>
      ...
      <div>
        <label className="block text-sm text-gray-600 mb-1">Frekvence</label>
        <select
          value={frequency}
      ...
      {frequency === 'weekly' && (
        <div>
          <label className="block text-sm text-gray-600 mb-1">Který den</label>
          <select
            value={targetDay}
      ...
      {frequency === 'monthly' && (
        <div>
          <label className="block text-sm text-gray-600 mb-1">Den v měsíci</label>
          <select
            value={targetDay}
            onChange={e => setTargetDay(Number(e.target.value))}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
